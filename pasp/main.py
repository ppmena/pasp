...
def install_help():
    ...
    ui.display_info("pip install git+https://github.com/ppmena/pasp")
...
def list_examples():
    ...
    # Try multiple common locations for examples
    possible_paths = [
...
def main():
    # Helper list for command recognition
...
Usage Examples:
  Style A (Direct File):
    ...
    pasp data.csv --anova score group        Run One-way ANOVA
    pasp data.csv --ttest-ind score gender --plot   Independent samples T-Test with visual plot
  ...
  Style B (Subcommands):
    ...
    # Check if first arg is a subcommand or a help/update flag
    ...
    # If first arg is not a subcommand and not a help flag, it might be a file (Style A)
    if first_arg and first_arg not in SUBCOMMANDS and not first_arg.startswith("-"):
        # Style A: pasp [file] [flags]
        ...
        if args.ttest_ind:
            try:
                ...
                if args.plot:
                    ...
                    g1_name, g2_name = df[args.ttest_ind[1]].unique()[:2]
                    g1 = df[df[args.ttest_ind[1]] == g1_name][args.ttest_ind[0]]
                    g2 = df[df[args.ttest_ind[1]] == g2_name][args.ttest_ind[0]]
                    plots.render_comparative_histogram(g1, g2, str(g1_name), str(g2_name))
    ...
    tt1_p.add_argument("file", help="Data file")
    tt1_p.add_argument("var", help="Variable")
    tt1_p.add_argument("value", type=float, help="Null value")

    tti_p = subparsers.add_parser("ttest-ind", help="Independent T-test")
    tti_p.add_argument("file", help="Data file")
    tti_p.add_argument("var", help="Variable")
    tti_p.add_argument("group", help="Grouping variable")
    tti_p.add_argument("--plot", action="store_true", help="Visual plot")

    ttp_p = subparsers.add_parser("ttest-paired", help="Paired T-test")
    ttp_p.add_argument("file", help="Data file")
    ttp_p.add_argument("var1", help="Var 1")
    ttp_p.add_argument("var2", help="Var 2")

    anova_p = subparsers.add_parser("anova", help="One-way ANOVA")
    anova_p.add_argument("file", help="Data file")
    anova_p.add_argument("var", help="Variable")
    anova_p.add_argument("group", help="Grouping variable")

    corr_p = subparsers.add_parser("correlation", help="Pearson correlation")
    corr_p.add_argument("file", help="Data file")
    corr_p.add_argument("vars", nargs="+", help="Variables")

    reg_p = subparsers.add_parser("regression", help="Linear regression")
    reg_p.add_argument("file", help="Data file")
    reg_p.add_argument("dep", help="Dependent")
    reg_p.add_argument("indep", help="Independent")

    subparsers.add_parser("doctor", help="Check system")
    subparsers.add_parser("install-help", help="Installation guide")
    subparsers.add_parser("examples", help="List examples")
    ...
    if args.command == "descriptives":
        ...
        try:
            res = stats.ttest_independent(df, args.var, args.group)
            ui.display_ttest(res)
            if args.plot:
                from pasp import plots
                gn = df[args.group].unique()[:2]
        ...
        ui.display_ttest(stats.ttest_paired(df, args.var1, args.var2))
    elif args.command == "anova":
        ui.display_anova(stats.anova_oneway(df, args.var, args.group))
    elif args.command == "correlation":
        ui.display_header("Correlation Matrix")
        ui.display_table(stats.correlation(df, args.vars))
    elif args.command == "regression":
        ui.display_dict_as_table(stats.linear_regression(df, args.dep, args.indep), title="Linear Regression")
